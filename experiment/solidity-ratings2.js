var jsPsych = initJsPsych({
    ...
    on_finish: function(){
      ...
      // Add this code at the end of your solidity-ratings.js file

// Get the trials data in the desired format
const trialsData = jsPsych.data.get().json();
const formattedData = jsPsych.data.get().json() ;

// Modify the data structure to match your desired format
const formattedData1 = trialsData.map(trial => {
    const formattedTrial = {
        ...
        // Add other properties based on your specific trial types
    };

    if (trial.trial_type === 'instructions') {
        formattedTrial.view_history = [
            {
                page_index: 0,
                viewing_time: trial.time_elapsed,
            },
        ];
    } else if (trial.trial_type === 'html-button-response') {
        formattedTrial.stimulus = trial.stimulus;
        formattedTrial.response = trial.response;
    } else if (trial.trial_type === 'survey-multi-choice') {
        formattedTrial.response = trial.response;
        formattedTrial.question_order = trial.question_order;
    }

...
full_data %>% group_by(theword, block) %>% summarize( count = n()) %>% ungroup() %>% mutate(mean = sum(count)/ n())

print(paste("mean number of ratings per word",  6.926))
...
raters <- colnames(ratings_wide1)[-1]  # exclude "theword" 
pairs <- combn(raters, 2, simplify = FALSE)
